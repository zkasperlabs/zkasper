...
| `recent_latencies[].blocked_millis` | more than a few seconds | The chain crossed while the prover was still finishing a proof it started before, so the epoch was held for the rest of that proof. It is the backlog the epoch opened with, and it is the schedule's problem rather than the trigger's: nothing caps it and no faster trigger removes it. **This was reported as `wait_millis` until 2026-08-19** — 27-30 s of it, against a 10 s cap — so a wait far above the cap in an older row is this. Read it with `late_group_millis`, which is the same backlog when it lands after the fire instead of before it. |
| `recent_latencies[].wait_millis` | above `--max-trigger-wait-millis` | The trigger holding for in-flight attestations, and nothing else — a **79 ms median** on the live run. It is bounded by the cap plus one trigger interval, and by what `tail_named` is worth, so a value past either is a proof being charged to the trigger again and not a trigger that held. |
| `recent_latencies[].observation_millis` | more than one proof | The daemon is not seeing the chain cross until after whatever it was proving finishes, because the trigger is only evaluated on a tick with no proof in flight. It is a real part of `T2 - T` and the largest one on a saturated daemon — a median 99 s on the 2026-08-19 mainnet run. It is also the only term a consumer pays that no faster prover would remove. |
