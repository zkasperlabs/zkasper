...
**This API serves finality proofs, and that is why it lives in `docs/finality/`.**
Every path is indexed by epoch, every stage name is a finality stage, and every
public input it publishes is a finality claim. There is no fast confirmation
surface in v1: an FCR confirmation is indexed by block root and slot, which this
index cannot express, so it would need endpoints of its own.
...
4. Check the proof: `zkasper_common::recursion::verify_child(&words,
   ...
   against the justification guest's. See [assumptions.md](assumptions.md),
