...
impl StreamPipeline {
    ...
    async fn open<A: BeaconApi + ChainStatusApi>(
        ...
        let context = StreamContext {
            ...
            target_root,
            signing_domain,
            group_program_vk: engine.prover.program_vk(Stage::Group),
            aggregate_program_vk: engine.prover.program_vk(Stage::Aggregate),
            previous_program_vk: match previous {
                PreviousJustification::Batch(_) => engine.prover.program_vk(Stage::Justification),
                PreviousJustification::Stream(_) => engine.prover.program_vk(Stage::StreamFinal),
            },
            epoch_diff_program_vk: engine.prover.program_vk(Stage::EpochDiff),
            committee_program_vk: engine.prover.program_vk(Stage::Committee),
